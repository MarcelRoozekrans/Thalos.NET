...
/// resolver backed by <c>IAgentCatalog</c>, which does not exist in this unit project. <see cref="AgentExistsAsync"/>
/// and <see cref="SkillExistsAsync"/> are not exercised by <see cref="WorkflowNodeDispatcher"/> (only
/// <see cref="ProcessValidator"/> uses them) but are implemented anyway so this type is a complete, honest
/// <see cref="IWorkflowReferenceResolver"/>.
/// </summary>
...
{
    public ValueTask<bool> AgentExistsAsync(string name, CancellationToken ct) =>
        ValueTask.FromResult(agentsByName.ContainsKey(name));
